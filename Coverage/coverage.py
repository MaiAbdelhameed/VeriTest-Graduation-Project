...
def Branch_Coverage_Optimization(Branches_conditions, input_test_cases, input_output_wire, CODE_GRAPH, G, Blocks_and_Conditions, Testcases_Blocks_Coverage):
    ...
    for key, branch_cond in Branches_conditions.items():
        ...
        blocks_covered, Branches_conditions = traverse(root, -1,blocks_covered, Branches_conditions, G, input_test_cases, Blocks_and_Conditions, Testcases_Blocks_Coverage, statements_covered, explore = True)
        blocks_covered -= 1 
...
def traverse(node, testcase_id,blocks_covered, Dict_of_Branches, G, input_test_cases, Blocks_and_Conditions, Testcases_Blocks_Coverage, statements_covered, explore):
    # Evaluate the condition using the input test cases
        ...
        if final_result is True: # If the block is covered increment blocks covered and mark its statements as covered
            ...
            for child in G.successors(node):
                blocks_covered,Dict_of_Branches = traverse(child,testcase_id,blocks_covered,Dict_of_Branches, G, input_test_cases, Blocks_and_Conditions, Testcases_Blocks_Coverage, statements_covered, explore= True)
        else:
            for child in G.successors(node):
                blocks_covered,Dict_of_Branches = traverse(child,testcase_id,blocks_covered,Dict_of_Branches, G, input_test_cases, Blocks_and_Conditions, Testcases_Blocks_Coverage, statements_covered, explore= False)
...
def COVERAGE_MAIN(path):
    ...
    input_test_cases, input_output_wire = random_stimuli(num_testcases, path) ## uncomment
    ...
    blocks_covered, Dict_of_Branches = traverse(root_block, 0, blocks_covered, Dict_of_Branches, G, input_test_cases, Blocks_and_Conditions, Testcases_Blocks_Coverage, [], explore = True)
    ...
    unique_coverage = {}
    redundant_testcases = []
    optimize_testset(input_test_cases, Testcases_Blocks_Coverage, unique_coverage, redundant_testcases)
